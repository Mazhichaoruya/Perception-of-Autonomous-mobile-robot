...
int FilterGndForPos_cor(float* outPoints,float*inPoints,int inNum)
{
    ...
    std::cout << "outNum " << outNum << "\n";
    // std::cout << inNum << " " << imgMinZ << "free \n";
    free(imgMinZ);
    // std::cout << imgMaxZ << " free0.1 \n";
    // imgMaxZ[0] = 1;
    // std::cout << imgMaxZ << " free0.11 \n";
    free(imgMaxZ);
    // std::cout << imgSumZ << " free0.2 \n";
    free(imgSumZ);
    // std::cout << imgMeanZ << " free0.3 \n";
    free(imgMeanZ);
    // std::cout << imgNumZ << " free0.4 \n";
    free(imgNumZ);
    // std::cout << idtemp << " free1 \n";
    free(idtemp);
    //std::cout << "free2 \n";
    return outNum;
}
...
int CalGndPos_cor(float *gnd, float *fPoints,int pointNum,float fSearchRadius)
{
    ...
    //     std:: cout << "before kdtree" << cloud->points.size() << std::endl;
    pcl::KdTreeFLANN<pcl::PointXYZ> kdtree;
    kdtree.setInputCloud (cloud);
    int nNum=0;
    //std::cout << "after setinput\n";
    ...
    for(int pid=0;pid<pointNum;pid++)
    {
        ...
        if ((nNum<1000)&&(pLabel[pid]==0))
        {
            ...
            if(GetNeiborPCA_cor(npca,cloud,kdtree,searchPoint,fSearchRadius)>0)
            {
                ...
                //std::cout << "in PCA3\n";
                // cout<<npca.eigenValuesPCA[1]/npca.eigenValuesPCA[0]<<" "<<npca.eigenVectorsPCA(0,0)<<endl;
...
int GetGndPos(float *pos, float *fPoints,int pointNum){
   ...
   // cout<<"tmp: "<<tmpPos[0]<<" "<<tmpPos[1]<<" "<<tmpPos[2]<<" "<<tmpPos[3]<<" "<<tmpPos[4]<<" "<<tmpPos[5]<<endl;
    if(gnd_pos[5]==0){
...
/*int Ori2Corrected(float *fPoints,int pointNum){
    ...
    // int vnum=CalNomarls(nvects,fPoints3,pnum3,1.0);
    float tmpPos[6];
    // cout<<sizeof(tmpPos)<<" "<<sizeof(gnd_pos)<<endl;
    
    int gndnum = CalGndPos_cor(tmpPos,fPoints3,pnum3,1.0);
    
    // cout<<"tmp: "<<tmpPos[0]<<" "<<tmpPos[1]<<" "<<tmpPos[2]<<" "<<tmpPos[3]<<" "<<tmpPos[4]<<" "<<tmpPos[5]<<endl;
    // cout<<"gnd: "<<gnd_pos[0]<<" "<<gnd_pos[1]<<" "<<gnd_pos[2]<<" "<<gnd_pos[3]<<" "<<gnd_pos[4]<<" "<<gnd_pos[5]<<endl;
    if(gnd_pos[5]==0){
        memcpy(gnd_pos,tmpPos,sizeof(tmpPos));
    }
    else{
        if(gndnum>0&&abs(gnd_pos[0]-tmpPos[0])<0.3){
            // cout<<"-----------------------ok---------------------"<<endl;
            memcpy(gnd_pos,tmpPos,sizeof(tmpPos));
        }
    }
    CorrectPoints_cor(fPoints,pointNum,gnd_pos);
    free(fPoints3);
}*/
